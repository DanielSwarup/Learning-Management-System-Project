<template>
        <div class="dashboard-cards-cont">
            <div class="dashboard-grades-cont" v-for="courses in allCourses" :key="courses.courseName">
...
<svg xmlns="http://www.w3.org/2000/svg" width="25.969" height="38" viewBox="0 0 25.969 38">
  <path id="Icon_material-class" data-name="Icon material-class" d="M28.723,3H9.246A3.567,3.567,0,0,0,6,6.8V37.2A3.567,3.567,0,0,0,9.246,41H28.723a3.567,3.567,0,0,0,3.246-3.8V6.8A3.567,3.567,0,0,0,28.723,3ZM9.246,6.8h8.115V22L13.3,19.15,9.246,22Z" transform="translate(-6 -3)"/>
                ...
                <ul class = "dash-calendar-items" >
                    <li >
                        <div class = "dash-calendar-element">
                            ...
                            <div class="dash-calendar-nameHead">
                                <b>Quiz Name</b> (Click Specific Quiz To find out more)
                            </div>
                    ...
                    <li v-for="items in getSpecificAssesments(courses)" v-bind:key='items.courseId'>
                        <router-link :to="{ name: 'QuizGrades', params: { courseName: courses.courseName, quiz: items.name }}" style="text-decoration:none">
                    ...
                    <li v-if="getSpecificAssesments(courses) == ''">
                        <div class = "dash-calendar-element">
...
export default {
    ...
    methods: {
        getSpecificAssesments(course){
            var specificAssesments = []
            // var counter = 0
            for(var i = 0; i<this.allQuizes.length; i++){
                if(this.allQuizes[i].courseId == course._id){
                    specificAssesments.push(this.allQuizes[i])
                }
            }
            // for(var j = 0; j<this.allUsers.length; j++){
            //     for(var k = 0; k<this.allUsers[i].grades.length; k++){
            //         if(this.allUsers[i].grades[i].courseId == course._id && this.allUsers[i].grades[i].name ){
            //             console.log("true")
            //             counter ++;
            //         }
            //     }
        ...
        courseTotal : function(courseId){
            ...
            // }
            object = { style:"width: " + "55" + "%", grade: "55" + "%" }
            return object             
        },
