...
<path id="Path_8" data-name="Path 8" d="M4.5,15H37.4" transform="translate(-4.5 -0.38)" fill="none" stroke="#000" stroke-linecap="round" stroke-linejoin="round" stroke-width="3"/>
            ...
            <ul class = "dash-calendar-items" >
                <li v-for="item in filteredCalendar" :key="item.assignmentName">
                ...
                <li v-if="filteredCalendar.length == 0">
                    <div class = "dash-calendar-element">
                        <div class = "dash-calendar-dateCont">
        ...
        <div class = "dashboard-course-cont">
            <ul class = "dashboard-course-card"> 
                <li  v-for="courses in userCourses" :key="courses.courseName">   
                    <router-link :to="{ name: 'Course', params: { name: courses.courseName}}" style="text-decoration:none">
...
<path id="Icon_material-class" data-name="Icon material-class" d="M47.046,3H11.864A5.881,5.881,0,0,0,6,8.864v46.91a5.881,5.881,0,0,0,5.864,5.864H47.046a5.881,5.881,0,0,0,5.864-5.864V8.864A5.881,5.881,0,0,0,47.046,3ZM11.864,8.864H26.523V32.319l-7.33-4.4-7.33,4.4Z" transform="translate(-6 -3)"/>
                            ...
                            <div class="dashboard-course-grade"></div>
                            <div class="dashboard-course-recent-update">Add Course</div>
                            <div class="dashboard-course-update-info">Click "Join new course" on sidebar</div>
                    </div>
                </li>
            </ul>
        </div>
...
export default {
    ...
    async created() {
        await getUserInfo()
        await getUserCoursesInfo()
        this.userCourses = userCourseInfo
        await this.getCalendarItems()
        await this.fitlerCalendarItems()
    },
    methods:{
        fitlerCalendarItems: function(){
            for(var i = 0; i<this.calendarItems.length; i++){
                for(var j = 0; j< this.userCourses.length; j++){
                    if(this.userCourses[j]._id == this.calendarItems[i].courseId){
                        this.filteredCalendar.push(this.calendarItems[i])
                    }
                }
            }
        },
...
async function getUserInfo(){
    var userCookieID = decodeURIComponent( getCookie('UserID')).replace('j:','')
    await fetch('http://localhost:7000/graphql', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        ...
        // credentials: 'include',
        body: JSON.stringify({query: `query {userById(_id: ${userCookieID}){role coursesId}}`})
    })
    .then( result => result.json())
    .then(function(data){
      userInfo = (data.data.userById)
...
var userCourseInfo
async function getUserCoursesInfo(){
  await fetch('http://localhost:7000/graphql', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        ...
        // credentials: 'include',
        body: JSON.stringify({query: `query {  courseByIds ( _ids: ${JSON.stringify(userInfo.coursesId)}){ _id courseName tutor annoucement }   }`})
    })
    .then( result => result.json())
    .then(function(data){

      userCourseInfo = (data.data.courseByIds)
